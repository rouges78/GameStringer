...
export function detectFormat(content: string, filename?: string): FileFormat {
  ...
  // Detect CSV by content: multiple lines with consistent comma/tab separators
  const lines = trimmed.split('\n').filter(l => l.trim());
  if (lines.length >= 2) {
    const firstLine = lines[0].toLowerCase();
    
    // Check for common CSV headers (key,value format)
    if (firstLine.includes('key,value') || firstLine.includes('id,text') || 
        firstLine.includes('g2u_') || firstLine.match(/^[a-z_]+,[a-z_]+$/i)) {
      return 'csv';
    }
    
    // Check for comma-separated or tab-separated values
    const commaCount = (lines[0].match(/,/g) || []).length;
    const tabCount = (lines[0].match(/\t/g) || []).length;
    
    // Accept files with at least 1 comma (2 columns) or 1 tab
    if (commaCount >= 1 || tabCount >= 1) {
      // Verify consistency with second line
      const secondLine = lines[1];
      const commaCount2 = (secondLine.match(/,/g) || []).length;
      const tabCount2 = (secondLine.match(/\t/g) || []).length;
      
      // Check if separator count is consistent (within 1 difference for quoted values)
      if ((commaCount >= 1 && Math.abs(commaCount - commaCount2) <= 1) || 
          (tabCount >= 1 && Math.abs(tabCount - tabCount2) <= 1)) {
        return 'csv';
      }
    }
  }
...
export function parseCSV(content: string, options: { delimiter?: string; hasHeader?: boolean } = {}): ParseResult {
  ...
  if (hasHeader) {
    ...
    // Find key column (various naming conventions)
    keyIndex = headers.findIndex(h => /^(key|id|name|g2u_id|msgid|source)$/i.test(h.replace(/[🔒]/g, '').trim()));
    // Find value column (prefer English for translation source)
    valueIndex = headers.findIndex(h => /^(value|text|string|translation|english|en|source_text)$/i.test(h.replace(/[🔒]/g, '').trim()));
