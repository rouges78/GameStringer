...
// ============================================================================
// EXPORT FORMATI STANDARD (TMX, XLIFF, PO)
// ============================================================================
...
/**
 * Raccoglie tutte le coppie sorgente/tradotta dai file
 */
function collectTranslationUnits(files: TranslatedFile[]): TranslationUnit[] {
  const units: TranslationUnit[] = [];
  for (const file of files) {
    const pairs = extractTranslationPairs(file.content, file.format);
    for (let i = 0; i < pairs.length; i++) {
      units.push({
        id: `${sanitizeFilename(file.relativePath)}_${i}`,
        source: pairs[i].original,
        target: pairs[i].translated,
...
/**
 * Esporta in formato TMX (Translation Memory eXchange)
 * Compatibile con OmegaT, Trados, memoQ, Memsource
 */
export function exportTMX(
  files: TranslatedFile[],
  metadata: PatchMetadata,
): string {
  const units = collectTranslationUnits(files);
  const srcLang = metadata.sourceLanguage.toLowerCase();
  const tgtLang = metadata.targetLanguage.toLowerCase();
  const lines: string[] = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    '<!DOCTYPE tmx SYSTEM "tmx14.dtd">',
    `<tmx version="1.4">`,
    `  <header creationtool="GameStringer" creationtoolversion="${metadata.version}"`,
    `    datatype="plaintext" segtype="sentence"`,
    `    adminlang="${srcLang}" srclang="${srcLang}"`,
    `    o-tmf="GameStringer"/>`,
    `  <body>`,
  ];
  for (const u of units) {
    lines.push(`    <tu tuid="${escXml(u.id)}">`);
    lines.push(`      <tuv xml:lang="${srcLang}"><seg>${escXml(u.source)}</seg></tuv>`);
    lines.push(`      <tuv xml:lang="${tgtLang}"><seg>${escXml(u.target)}</seg></tuv>`);
    lines.push(`    </tu>`);
  }
  lines.push(`  </body>`);
  lines.push(`</tmx>`);
 ...
 * Compatibile con Crowdin, Weblate, Lokalise
 */
export function exportXLIFF(
  ...
  const lines: string[] = [
    ...
    `  <file source-language="${srcLang}" target-language="${tgtLang}"`,
    `    datatype="plaintext" original="${escXml(metadata.gameName)}"`,
    `    tool-id="GameStringer" tool-name="GameStringer">`,
    `    <body>`,
  ];
  for (const u of units) {
    lines.push(`      <trans-unit id="${escXml(u.id)}"${u.file ? ` resname="${escXml(u.file)}"` : ''}>`);
    lines.push(`        <source>${escXml(u.source)}</source>`);
    lines.push(`        <target>${escXml(u.target)}</target>`);
    if (u.context) lines.push(`        <note>${escXml(u.context)}</note>`);
...
/**
 * Esporta in formato GNU gettext PO
 * Compatibile con Weblate, Poedit, Launchpad
 */
export function exportPO(
  files: TranslatedFile[],
  metadata: PatchMetadata,
): string {
  const units = collectTranslationUnits(files);
  const lines: string[] = [
    `# ${metadata.gameName} — ${metadata.targetLanguage.toUpperCase()} Translation`,
    `# Generated by GameStringer`,
    `#`,
    `msgid ""`,
    `msgstr ""`,
    `"Project-Id-Version: ${metadata.gameName} ${metadata.version}\\n"`,
    `"Language: ${metadata.targetLanguage}\\n"`,
    `"Content-Type: text/plain; charset=UTF-8\\n"`,
    `"Content-Transfer-Encoding: 8bit\\n"`,
    `"X-Generator: GameStringer\\n"`,
    ``,
  ];
  for (const u of units) {
    if (u.file) lines.push(`#: ${u.file}`);
    const escSrc = u.source.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n');
    const escTgt = u.target.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n');
    lines.push(`msgid "${escSrc}"`);
    lines.push(`msgstr "${escTgt}"`);
    lines.push(``);
  }
  return lines.join('\n');
}
