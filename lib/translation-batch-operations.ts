...
function parseJSONImport(data: string): unknown[] {
  return JSON.parse(data);
}

function parseCSVImport(data: string): unknown[] {
...
function parseXLSXImport(data: unknown): unknown[] {
  // This would require xlsx library
  return data.worksheets?.[0]?.data || [];
}

function parsePOImport(data: string): unknown[] {
...
function validateImportItem(item: unknown): boolean {
  return !!(item.originalText && item.gameId && item.filePath);
}

function groupTranslationsByGame(translations: unknown[]): unknown {
