...
export async function batchTranslateProcessor(item: TranslationBatchItem): Promise<unknown> {
...
export async function batchStatusUpdateProcessor(item: TranslationBatchItem): Promise<unknown> {
...
export async function batchDeleteProcessor(item: TranslationBatchItem): Promise<unknown> {
...
export async function batchApproveProcessor(item: TranslationBatchItem): Promise<unknown> {
...
export async function batchRejectProcessor(item: TranslationBatchItem): Promise<unknown> {
...
// Translation record shape from Prisma
...
function formatAsJSON(translations: unknown[], options: ExportOptions): string {
...
function formatAsCSV(translations: unknown[], _options: ExportOptions): string {
  const headers = [
    'ID',
    'Game',
    'File Path',
  ...
  const rows = (translations as TranslationRecord[]).map(t => [
    t.id,
    t.game?.title || '',
    t.filePath,
...
function formatAsXLSX(translations: unknown[], _options: ExportOptions): unknown {
  // This would require a library like xlsx
  // For now, return structured data that can be processed by xlsx
  return {
    worksheets: [{
      name: 'Translations',
      data: (translations as TranslationRecord[]).map(t => ({
        ID: t.id,
        Game: t.game?.title || '',
...
function formatAsPO(translations: unknown[], _options: ExportOptions): string {
  const typed = translations as TranslationRecord[];
  let po = `# Translation file generated by GameStringer
# Generated on: ${new Date().toISOString()}
msgid ""
msgstr ""
"Content-Type: text/plain; charset=UTF-8\\n"
"Language: ${typed[0]?.targetLanguage || 'unknown'}\\n"
...
function parseJSONImport(data: unknown): ImportItem[] {
  return JSON.parse(String(data));
}

function parseCSVImport(data: unknown): ImportItem[] {
  ...
  return lines.slice(1).map(line => {
    ...
    return item as unknown as ImportItem;
  });
}

function parseXLSXImport(data: unknown): ImportItem[] {
  // This would require xlsx library
  const d = data as Record<string, { data?: ImportItem[] }[]> | undefined;
  return d?.worksheets?.[0]?.data || [];
}

function parsePOImport(data: unknown): ImportItem[] {
  const translations: ImportItem[] = [];
  const lines = String(data).split('\n');
  ...
  for (const line of lines) {
    if (line.startsWith('msgid ')) {
      ...
      if (currentEntry.originalText) {
        translations.push({ ...currentEntry } as unknown as ImportItem);
        currentEntry = {};
      }
    }
  }
...
function validateImportItem(item: ImportItem): boolean {
  return !!(item.originalText && item.gameId && item.filePath);
}

function groupTranslationsByGame(translations: unknown[]): Record<string, unknown[]> {
  return (translations as TranslationRecord[]).reduce<Record<string, unknown[]>>((acc, translation) => {
