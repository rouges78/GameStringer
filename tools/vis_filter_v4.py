"""Test v4 filter with binary prefix rejection"""
...
def has_early_control(b, check=8):
    for i in range(min(len(b), check)):
        if b[i] == 0 or (b[i] < 0x20 and b[i] not in (9, 10, 13)):
            return True
    return False
...
def filt(s):
    ...
    if lower and not lower[0].isalnum():
        trimmed = lower.lstrip(lower[0])
        for p in engine_pfx:
    ...
    if 'clickbehaviour' in lower or 'clickbehavior' in lower: return False
    ...
    if any('_' in w for w in s.split()): return False
...
print("Extracting with v4 (binary prefix rejection + filter)...")
strings = []
garbage_rejected = 0
p = 0
while p + 4 < len(payload):
    ...
    if 2 <= length <= 10000 and p + 4 + length <= len(payload):
        ...
        # NEW: reject if first byte is control/null
        if raw[0] < 0x20 and raw[0] not in (9, 10, 13):
            p += 1
            continue
        # NEW: reject if any of first 8 bytes are null/control
        if has_early_control(raw):
            garbage_rejected += 1
            p += 1
            continue
        try:
            text = raw.decode('utf-8').rstrip('\x00')
            # NEW: reject embedded nulls
            if '\x00' in text:
                garbage_rejected += 1
                p += 1
                continue
...
print(f"Total: {len(strings)} (rejected {garbage_rejected} with binary prefix)")

# Check for any remaining strings with suspicious chars
suspect = 0
for t in strings:
    if any(ord(c) < 0x20 and c not in '\t\n\r' for c in t):
        suspect += 1
        if suspect <= 5:
            print(f"  SUSPECT: {repr(t[:100])}")
print(f"Strings with control chars: {suspect}")

print(f"\nFirst 30:")
for i, t in enumerate(strings[:30]):
    print(f"  [{i:4d}] {t[:120]}")
print(f"\nMiddle sample (3000-3020):")
for i, t in enumerate(strings[3000:3020]):
    print(f"  [{3000+i:4d}] {t[:120]}")
