...
export function GlossaryManager({
  ...
  const addEntry = async () => {
    if (!newEntry.original || !newEntry.translation) {
      toast.error('Enter original term and translation');
      return;
    }
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Create a glossary to maintain translation consistency
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button onClick={createGlossary} className="w-full">
            <Plus className="mr-2 h-4 w-4" />
            Create Glossary for {gameName}
          </Button>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              aria-label="Search" placeholder="Search term..."
              value={searchTerm}
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Original ({glossary.sourceLanguage.toUpperCase()})</Label>
                    <Input
                  ...
                  <div>
                    <Label>Translation ({glossary.targetLanguage.toUpperCase()})</Label>
                    <Input
                      value={newEntry.translation}
                      onChange={(e) => setNewEntry({ ...newEntry, translation: e.target.value })}
                      placeholder="e.g. Life Points"
                    />
                  </div>
                </div>
          ...
          <div className="space-y-2">
            {filteredEntries.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                {searchTerm ? 'No results' : 'No terms in glossary'}
              </div>
            ) : (
