          ...
          {selectedDict ? <>
            ...
            <CardContent className="flex-1 min-h-0 flex flex-col gap-2 p-3">
              <div className="flex items-center gap-2 text-xs">
                ...
                <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}><DialogTrigger asChild><Button variant="ghost" size="xs" className="px-2 text-xs"><Plus className="h-3 w-3 mr-1" />{t('dictionaryManager.add')}</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{t('dictionaryManager.addTranslation')}</DialogTitle></DialogHeader><div className="grid gap-3 py-4"><div><label className="text-xs text-gray-400 mb-1 block">{t('dictionaryManager.original')}</label><Textarea value={newTrans.original} onChange={e=>setNewTrans({...newTrans,original:e.target.value})} rows={2} /></div><div><label className="text-xs text-gray-400 mb-1 block">{t('dictionaryManager.translation')}</label><Textarea value={newTrans.translated} onChange={e=>setNewTrans({...newTrans,translated:e.target.value})} rows={2} /></div></div><DialogFooter><Button variant="outline" onClick={()=>setAddDialogOpen(false)}>{t('dictionaryManager.cancel')}</Button><Button onClick={handleAdd}>{t('dictionaryManager.save')}</Button></DialogFooter></DialogContent></Dialog>
                <Button size="xs" className="px-2 text-xs bg-indigo-600 hover:bg-indigo-700" onClick={handleApply}><Zap className="h-3 w-3 mr-1" />{t('dictionaryManager.apply')}</Button>
