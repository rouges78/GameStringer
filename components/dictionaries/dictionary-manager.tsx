...
export function DictionaryManager() {
  ...
  const handleDelete = async (d: DictionaryInfo) => {
    ...
    if ((await deleteDictionary(d.id, d.target_language)).success) { toast({ title: t('dictionaryManager.deleted') }); loadData(); if (selectedDict?.id === d.id) setSelectedDict(null); }
  };

  const handleImport = async () => {
    if (!importFilePath) {
      toast({ title: t('dictionaryManager.error'), description: t('dictionaryManager.selectJsonFile'), variant: 'destructive' });
      return;
    }
    const r = await importDictionaryAuto(importFilePath);
    if (r.success) { 
      toast({ title: t('dictionaryManager.imported'), description: `${r.entries_loaded} ${t('dictionaryManager.entriesLoaded')}` }); 
      setImportDialogOpen(false); 
      setImportFilePath('');
      loadData(); 
    } else {
      toast({ title: t('dictionaryManager.error'), description: r.message, variant: 'destructive' });
    }
  };
  ...
  const handleAdd = async () => {
    ...
    if ((await addTranslationToDictionary(selectedDict.id, selectedDict.target_language, newTrans.original, newTrans.translated)).success) {
      toast({ title: t('dictionaryManager.added') }); setAddDialogOpen(false); setNewTrans({ original: '', translated: '' }); loadData();
    }
  };
  ...
  const handleApply = async () => {
    ...
    if (applyDialogOpen && gamePath) {
      ...
      if (r.success) { 
        toast({ title: t('dictionaryManager.applied'), description: `${t('dictionaryManager.dictionaryCopiedTo')} ${gamePath}` }); 
    ...
    if (r.success) { 
      toast({ title: t('dictionaryManager.applied'), description: `${t('dictionaryManager.dictionaryCopiedTo')} ${path}` }); 
    } else {
      toast({ title: t('dictionaryManager.error'), description: r.message, variant: 'destructive' });
    }
  };
    ...
    <div className="flex flex-col h-[calc(100vh-140px)]">
      <div className="flex items-center justify-between rounded-xl bg-gradient-to-r from-indigo-600/20 to-purple-600/20 border border-indigo-500/30 p-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="text-lg font-bold">{t('dictionaryManager.title')}</h2><p className="text-xs text-indigo-300/70">{t('dictionaryManager.subtitle')}</p></div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
            <DialogTrigger asChild><Button size="sm" className="bg-indigo-600 hover:bg-indigo-700"><Upload className="h-4 w-4 mr-1.5" />{t('dictionaryManager.import')}</Button></DialogTrigger>
            <DialogContent className="sm:max-w-lg border-indigo-500/30 bg-gradient-to-b from-gray-900 to-gray-950">
              <DialogHeader className="pb-4 border-b border-gray-800">
                <DialogTitle className="flex items-center gap-3 text-lg">
                  ...
                  {t('dictionaryManager.importTitle')}
                </DialogTitle>
                <p className="text-sm text-gray-400 mt-1">{t('dictionaryManager.importDesc')}</p>
              </DialogHeader>
              <div className="py-6">
                <div 
                  onClick={pickFile}
                ...
                >
                  {importFilePath ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-sm font-medium text-emerald-400">{t('dictionaryManager.fileSelected')}</p>
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm font-medium text-gray-300">{t('dictionaryManager.clickToSelect')}</p>
                      <p className="text-xs text-gray-500">{t('dictionaryManager.fileMustContain')}</p>
                    </div>
                  )}
                </div>
              </div>
              <DialogFooter className="pt-4 border-t border-gray-800 gap-2">
                <Button variant="ghost" onClick={()=>{setImportDialogOpen(false);setImportFilePath('');}} className="text-gray-400">{t('dictionaryManager.cancel')}</Button>
                <Button onClick={handleImport} className="bg-indigo-600 hover:bg-indigo-700 px-6" disabled={!importFilePath}>
                  <FileUp className="h-4 w-4 mr-2" />{t('dictionaryManager.import')}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
      ...
      <div className="grid grid-cols-12 gap-4 flex-1 min-h-0">
        <Card className="col-span-5 border-gray-800 bg-gray-900/50 flex flex-col">
          <CardHeader className="py-3 px-4 border-b border-gray-800"><CardTitle className="text-sm flex items-center gap-2"><Layers className="h-4 w-4 text-indigo-400" />{t('dictionaryManager.installed')} ({dictionaries.length})</CardTitle></CardHeader>
          <CardContent className="p-0 flex-1 min-h-0">
            <ScrollArea className="h-full">
              {loading ? <div className="flex justify-center py-12"><RefreshCw className="h-6 w-6 animate-spin text-gray-500" /></div>
              : dictionaries.length === 0 ? <div className="flex flex-col items-center py-12"><Book className="h-12 w-12 text-gray-700 mb-3" /><p className="text-gray-400">{t('dictionaryManager.noDictionaries')}</p></div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-amber-300/60 mt-0.5">{getLanguageFlag(d.source_language)} → {getLanguageFlag(d.target_language)} • {d.entries_count.toLocaleString()} {t('dictionaryManager.entries')}</div>
                    </div>
        ...
        <Card className="col-span-7 border-gray-800 bg-gray-900/50 flex flex-col">
          {selectedDict ? <>
            <CardHeader className="py-3 px-4 border-b border-gray-800 flex-row items-center justify-between">
              ...
              <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={()=>exportDictionarySimple(selectedDict.id,selectedDict.target_language,selectedDict.id+'_export.json')}><Download className="h-4 w-4 mr-2" />{t('dictionaryManager.export')}</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem className="text-red-400" onClick={()=>handleDelete(selectedDict)}><Trash2 className="h-4 w-4 mr-2" />{t('dictionaryManager.delete')}</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
            </CardHeader>
            <CardContent className="flex-1 min-h-0 flex flex-col gap-2 p-3">
              <div className="flex items-center gap-2 text-xs">
                <span className="bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded">{selectedDict.entries_count.toLocaleString()} {t('dictionaryManager.entries')}</span>
                ...
                <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}><DialogTrigger asChild><Button variant="ghost" size="sm" className="h-6 px-2 text-xs"><Plus className="h-3 w-3 mr-1" />{t('dictionaryManager.add')}</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>{t('dictionaryManager.addTranslation')}</DialogTitle></DialogHeader><div className="grid gap-3 py-4"><div><label className="text-xs text-gray-400 mb-1 block">{t('dictionaryManager.original')}</label><Textarea value={newTrans.original} onChange={e=>setNewTrans({...newTrans,original:e.target.value})} rows={2} /></div><div><label className="text-xs text-gray-400 mb-1 block">{t('dictionaryManager.translation')}</label><Textarea value={newTrans.translated} onChange={e=>setNewTrans({...newTrans,translated:e.target.value})} rows={2} /></div></div><DialogFooter><Button variant="outline" onClick={()=>setAddDialogOpen(false)}>{t('dictionaryManager.cancel')}</Button><Button onClick={handleAdd}>{t('dictionaryManager.save')}</Button></DialogFooter></DialogContent></Dialog>
                <Button size="sm" className="h-6 px-2 text-xs bg-indigo-600 hover:bg-indigo-700" onClick={handleApply}><Zap className="h-3 w-3 mr-1" />{t('dictionaryManager.apply')}</Button>
                <Dialog open={applyDialogOpen} onOpenChange={setApplyDialogOpen}><DialogContent><DialogHeader><DialogTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-indigo-400" />{t('dictionaryManager.gameNotFound')}</DialogTitle></DialogHeader><div className="py-4"><p className="text-sm text-gray-400 mb-3">{t('dictionaryManager.selectGameFolder')}</p><div className="flex gap-2"><Input value={gamePath} onChange={e=>setGamePath(e.target.value)} placeholder={t('dictionaryManager.selectFolder')} className="flex-1" /><Button variant="outline" onClick={pickFolder}><FolderInput className="h-4 w-4" /></Button></div></div><DialogFooter><Button variant="outline" onClick={()=>{setApplyDialogOpen(false);setGamePath('');}}>{t('dictionaryManager.cancel')}</Button><Button onClick={handleApply} disabled={!gamePath} className="bg-indigo-600"><CheckCircle className="h-4 w-4 mr-1.5" />{t('dictionaryManager.apply')}</Button></DialogFooter></DialogContent></Dialog>
              </div>
              <div className="relative"><Input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder={t('dictionaryManager.searchTranslations')} className="bg-gray-800/50 pr-10 h-8 text-sm" />{isSearching ? <RefreshCw className="absolute right-3 top-1/2 -translate-y-1/2 h-3 w-3 text-indigo-400 animate-spin" /> : <Search className="absolute right-3 top-1/2 -translate-y-1/2 h-3 w-3 text-gray-500" />}</div>
              <div className="flex-1 min-h-0 overflow-auto rounded border border-gray-800 bg-gray-900/30">
                {isSearching?<div className="flex items-center justify-center h-full text-gray-500 text-xs"><RefreshCw className="h-4 w-4 animate-spin mr-2" />{t('dictionaryManager.loading')}</div>:searchResults.length>0?<table className="w-full text-xs"><tbody>{searchResults.map(([orig,trans],i)=><tr key={i} className="border-b border-gray-800 hover:bg-gray-800/50"><td className="p-1.5 text-gray-300 w-1/2">{orig}</td><td className="p-1.5 w-1/2">{editingIdx===i?<input autoFocus value={editValue} onChange={e=>setEditValue(e.target.value)} onBlur={async()=>{if(editValue!==trans&&selectedDict){await addTranslationToDictionary(selectedDict.id,selectedDict.target_language,orig,editValue);setSearchResults(r=>r.map((x,j)=>j===i?[x[0],editValue]:x));}setEditingIdx(null);}} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();if(e.key==='Escape'){setEditingIdx(null);}}} className="w-full bg-gray-800 text-emerald-400 px-1 py-0.5 rounded border border-indigo-500 outline-none" />:<span onClick={()=>{setEditingIdx(i);setEditValue(trans);}} className="text-emerald-400 cursor-pointer hover:bg-gray-800 px-1 py-0.5 rounded block">{trans}</span>}</td></tr>)}</tbody></table>:<div className="flex items-center justify-center h-full text-gray-500 text-xs">{searchQuery?t('dictionaryManager.noResults'):t('dictionaryManager.emptyDictionary')}</div>}
              </div>
            </CardContent>
          </> : <div className="flex flex-col items-center justify-center h-full"><Eye className="h-12 w-12 text-gray-700 mb-3" /><p className="text-gray-400">{t('dictionaryManager.selectDictionary')}</p></div>}
        </Card>
      </div>
    </div>
  );
