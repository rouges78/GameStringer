...
function ProfileCard({ profile, isSelected, isCurrentProfile = false }: ProfileCardProps) {
  ...
  const handleAvatarChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setAuthError(t('profile.selectValidImage'));
      return;
    }

    // Verifica dimensione (max 2MB)
    if (file.size > 2 * 1024 * 1024) {
      setAuthError(t('profile.imageTooLarge'));
      return;
    }
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        if (success) {
          ...
          setAuthError(t('profile.avatarUpdateError'));
        }
        setIsUpdatingAvatar(false);
      };
      reader.onerror = () => {
        setAuthError(t('profile.fileReadError'));
        setIsUpdatingAvatar(false);
      };
  ...
  const handleAuthenticate = async () => {
    if (!password.trim()) {
      setAuthError(t('profile.enterPassword'));
      return;
    }
  ...
  const handleDeleteProfile = async () => {
    if (!password.trim()) {
      setAuthError(t('profile.enterPasswordToDelete'));
      return;
    }
    ...
    if (!success) {
      setAuthError(t('profile.incorrectPasswordOrDeletion'));
    } else {
      // Reset UI on success
                    ...
                    >
                      {isAuthenticating ? (
                        <>
                          ...
                          <span className="text-sm">{t('profile.loggingIn')}</span>
                        </>
                      ) : (
                        <>
                          <span className="text-sm font-medium">{t('profile.login')}</span>
                    ...
                    {!showDeleteConfirm ? (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          if (isCurrentProfile) {
                            setAuthError(t('profile.cannotDeleteActive'));
                          } else {
                        ...
                        title={isCurrentProfile ? t('profile.cannotDeleteActive') : t('profile.deleteConfirm').replace(/<[^>]*>/g, '').replace('{name}', profile.name)}
                      >
                  ...
                  {showDeleteConfirm && (
                    ...
                    >
                      <p className="text-xs text-red-200/80 leading-relaxed">
                        {t('profile.deleteConfirm').replace('{name}', profile.name).split(/<\/?strong>/).map((part: string, i: number) => i === 1 ? <strong key={i}>{part}</strong> : part)}
                      </p>
                      <div className="flex gap-2">
                        <Button
                          variant="ghost"
      ...
      <div className="min-h-screen flex items-center justify-center relative">
        ...
        <Card className="w-full max-w-md relative z-10">
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-muted-foreground">{t('profile.loadingProfiles')}</p>
          </CardContent>
        </Card>
      </div>
    );
            ...
            {profiles.length > 0 && (
              <div className="mt-4">
                <Badge variant="outline" className="px-3 py-1 rounded-full bg-slate-800/50 border-slate-700 text-slate-300 backdrop-blur-sm text-xs">
                  ...
                  {profiles.length} {profiles.length === 1 ? t('profile.profileAvailable') : t('profile.profilesAvailable')}
                </Badge>
              </div>
            )}
          </motion.div>
            ...
            {profiles.length === 0 && (
              ...
              >
                 <div className="h-full min-h-[140px] rounded-xl border-2 border-dashed border-slate-700 hover:border-indigo-500/50 bg-slate-900/30 hover:bg-slate-800/50 transition-all duration-300 flex flex-col items-center justify-center p-6 text-slate-400 hover:text-indigo-300">
                    ...
                    <span className="font-medium">{t('profile.createFirstProfile')}</span>
                 </div>
               </motion.div>
            )}
          </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <div className="font-medium">{t('profile.newProfile')}</div>
                    <div className="text-xs text-slate-500">{t('profile.addAnotherUser')}</div>
                  </div>
                </div>
              </Button>
            </motion.div>
