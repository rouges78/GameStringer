...
export function ErrorFallback({
  ...
  description = "Si è verificato un errore inaspettato nell'applicazione.",
...
ho riscontrato un errore nell'applicazione GameStringer.
      ...
      <Card className="w-full max-w-2xl">
        ...
        <CardContent className="space-y-6">
          ...
          {showReportButton && (
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              >
                ...
                {reportCopied ? 'Report Copiato!' : 'Copia Report Errore'}
              </Button>
          ...
          <div className="text-center text-sm text-muted-foreground">
            Se il problema persiste, contatta il supporto tecnico includendo l'Error ID.
          </div>
        </CardContent>
      </Card>
    </div>
...
export function ApiErrorFallback({
  ...
  const title = `Errore API (${statusCode || 'Unknown'})`;
  const description = `Errore durante la chiamata ${method} a ${endpoint || 'endpoint sconosciuto'}.`;
...
export function LoadingErrorFallback({
  ...
  const title = `Errore di Caricamento`;
  const description = `Impossibile caricare ${resource}. Controlla la connessione e riprova.`;
...
export function AuthErrorFallback({
  ...
  const title = `Errore di Autenticazione`;
  const description = `La tua sessione è scaduta o non hai i permessi necessari.`;
