...
export function InjektUIEnhanced() {
  ...
  // Update stats when selected process changes
  useEffect(() => {
    if (selectedProcess) {
      updateStats();
  ...
  // Save overlay config when it changes
  useEffect(() => {
            ...
            <CardContent className="relative z-10">
              <TranslationProfileManager
                ...
                onProfileSelect={(profile) => {
                  // Load translations from selected profile
                  ...
                  // Switch to translations tab to show loaded translations
                  setActiveTab('translations');
                  
                  // Show notification
                  alert(`Profile "${profile.gameName}" loaded with ${profile.translations.length} translations`);
                }}
              />
            </CardContent>
          </Card>
