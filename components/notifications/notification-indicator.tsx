...
export const NotificationIndicator: React.FC<NotificationIndicatorProps> = ({
  ...
  const getAriaLabel = () => {
    if (isLoading) return 'Caricamento notifiche...';
    if (!hasNotifications) return 'Nessuna notifica non letta. Clicca per aprire il centro notifiche.';
    
    const countText = unreadCount === 1 ? '1 notifica non letta' : `${unreadCount} notifiche non lette`;
    return `${countText}. Clicca per aprire il centro notifiche.`;
  };
