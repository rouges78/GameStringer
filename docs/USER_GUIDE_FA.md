...
## 🆕 جدید در نسخهٔ v1.15.0

- ⚙️ **پارامترهای استنتاج Ollama**: صفحهٔ جدید *Ollama Manager → «پیشرفته»* با ۳ پیش‌تنظیم (وفادار/متوازن/خلاق) و یک حالت حرفه‌ای برای تنظیم `temperature`، `top_p`، `top_k`، `repeat_penalty`، `num_ctx` و `seed` مدل‌های محلی.
- 🚀 **پروژه‌ها ↔ Patch Hub**: هر `.gspack` واردشده به‌عنوان یک پروژهٔ کامل‌شده ثبت می‌شود؛ از «پروژه‌ها» می‌توانید **اعمال روی بازی** (با پشتیبان `.bak`) یا **انتشار** ترجمهٔ از پیش پرشده در Patch Hub را انجام دهید.
- 💬 **ویجت بازخورد** در *تنظیمات → انجمن*: باگ‌ها یا ایده‌ها را گزارش دهید؛ زمینهٔ فنی (نسخه، پلتفرم، صفحه) به‌طور خودکار پیوست می‌شود.
- ⚡ **Patch Hub سریع‌تر**: کش محلی فهرست‌ها و محدودیت‌های ضدسوءاستفاده تا بتوانید بدون بارگذاری مجدد از سرور بین تب‌ها جابه‌جا شوید.

## 🆕 جدید در نسخه v1.12.0

- 👁️ ترجمه با زمینه بصری (VLM): هوش مصنوعی صفحه را می‌بیند و با توجه به زمینه ترجمه می‌کند و ابهام واژه‌ها را برطرف می‌کند (مثلاً "Chest" = صندوق یا قفسه سینه). محلی (Ollama) یا OpenAI/Gemini
- 🪞 گذر خوداصلاح‌گر (Reflection): یک گذر دوم اختیاری، ترجمه را بر اساس واژه‌نامه، لحن و یکدستی فقط در جای لازم بازبینی و اصلاح می‌کند
- 🧠 حافظه معنایی (RAG): حافظه ترجمه و واژه‌نامه از طریق embedding محلی بر پایه معنا تطبیق می‌یابند تا اصطلاحات در کل بازی یکدست بمانند
- ⚡ ترجمه زنده روان‌تر و پردازش بومی تصویر (برش/تغییر اندازه پیش از فراخوانی هوش مصنوعی)
- 🔌 ارائه‌دهندگان بیشتر به‌صورت آماده: DeepL، Groq، Cerebras، Cohere، Together، Fireworks، Hugging Face، Azure Translator، DashScope، LibreTranslate، Lingva، MyMemory
- 🧰 تنظیمات جدید برای تنظیم reflection، RAG معنایی و زمینه بصری (کیفیت در برابر سرعت/هزینه)

## 🆕 تازه‌های نسخهٔ v1.11.2

- **فروشگاه‌های بیشتر**: اکنون Humble App، Game Jolt و Big Fish Games به‌صورت خودکار شناسایی می‌شوند.
- **جست‌وجوی ترجمه**: از طریق PCGamingWiki بررسی می‌کند که آیا بازی زبان شما را دارد، به‌همراه لینک‌های جست‌وجوی پچ‌های ایتالیایی طرفداران.
- **انتشار در Patch Hub**: یک پروژهٔ کامل‌شده را در یک گام به Patch Hub انجمن بفرستید (پروژه‌ها → انتشار).
- **موتورهای جدید**: خط لولهٔ ابری برای TyranoScript و پارسر بازنویسی‌شدهٔ `.pck` که آرشیوهای واقعی Godot 4.4+ را می‌خواند.
- **Unity**: پل محلی Ollama برای XUnity (CustomTranslate)؛ دانلودها (BepInEx/XUnity/TMP/UABEA) از طریق GitHub API.
...
20. [پروژه‌ها و Patch Hub](#projects--patch-hub) *(به‌روزرسانی v1.15.0)*
21. [پارامترهای استنتاج Ollama](#ollama-inference-parameters) *(جدید v1.15.0)*
22. [ارسال بازخورد](#send-feedback) *(جدید v1.15.0)*
...
## Patch Hub (بسته‌های ترجمه انجمن)

Patch Hub یک بازارچه به سبک Steam Workshop برای به اشتراک‌گذاری و دانلود بسته‌های ترجمه انجمن است که توسط سرور انجمن GameStringer پشتیبانی می‌شود. آن را از طریق ورودی **Patch Hub** در نوار کناری (بخش نارنجی/کهربایی) باز کنید.

### مرور بسته‌ها
نمای اصلی، بسته‌های منتشرشده را همراه با جستجو و مرتب‌سازی (بیشترین دانلود، بالاترین امتیاز، به‌روزرسانی اخیر، میزان تکمیل) فهرست می‌کند. هر کارت بازی، زبان مبدأ←مقصد، درصد تکمیل، امتیاز و تعداد دانلود را نشان می‌دهد. روی یک بسته کلیک کنید تا صفحه جزئیات آن همراه با آمار، توضیحات، فایل‌های گنجانده‌شده و فهرست تغییرات باز شود.

### دانلود یک بسته
در صفحه جزئیات یک بسته، روی **Download** (دانلود) کلیک کنید. GameStringer تمام فایل‌های آن بسته را از سرور انجمن دریافت می‌کند و آن‌ها را به صورت محلی به عنوان یک بسته `.gspack` در کتابخانه بسته‌های شما (`Documents/GameStringer/packs`) ذخیره می‌کند. از آنجا می‌توانید بسته را مدیریت کنید و از صفحه جزئیات یک بازی آن را وارد کنید تا ترجمه اعمال شود.

### انتشار یک بسته
روی **Publish patch** (انتشار پچ) کلیک کنید تا فرم انتشار باز شود. نام بسته، بازی، زبان مبدأ و مقصد، یک توضیح و برچسب‌های اختیاری را وارد کنید و فایل(های) ترجمه خود را پیوست کنید. هنگامی که در Community Hub وارد شده‌اید، بسته در سرور انجمن بارگذاری می‌شود و پیش از آنکه به صورت عمومی قابل مشاهده شود، وارد صف بازبینی (moderation) می‌شود. اگر وارد نشده باشید، بسته به عنوان یک پیش‌نویس محلی نگه داشته می‌شود — وارد شوید و دوباره منتشر کنید تا آن را به صورت آنلاین به اشتراک بگذارید.

> انتشار آنلاین به یک حساب Community Hub نیاز دارد (جدا از پروفایل محلی شما). مرور و دانلود بدون حساب کاربری کار می‌کند.

---

## PROJECTS & PATCH HUB
## (پروژه‌ها و Patch Hub)

*(به‌روزرسانی در v1.15.0)*

صفحهٔ **«پروژه‌ها»** همهٔ بازی‌هایی را که در حال ترجمه‌شان هستید یا قبلاً ترجمه کرده‌اید در یک جا گرد می‌آورد — داده‌ها از Quality Scoring، واژه‌نامه‌ها، Translation Memory و کتابخانهٔ بازی گرفته می‌شوند. از اینجا کل چرخهٔ عمر یک ترجمه را مدیریت می‌کنید: باز کردن، اعمال، انتشار.

### وارد کردن `.gspack` ← پروژه

وقتی یک **بستهٔ `.gspack` را وارد می‌کنید** (دانلودشده از Patch Hub یا ساختهٔ خودتان)، GameStringer فقط آن را اعمال نمی‌کند: بلکه **یک پروژهٔ کامل‌شده ثبت می‌کند** و فایل‌های ترجمه‌شده را به‌صورت محلی **ذخیره می‌کند**. سپس آن بازی در «پروژه‌ها» با ۱۰۰٪ نمایش داده می‌شود و آمادهٔ اعمال مجدد یا انتشار بعدی است، بدون آنکه کاری را دوباره انجام دهید.

### اعمال روی بازی

برای پروژه‌های **کامل‌شده**، دکمهٔ **«اعمال روی بازی»** (آیکون پوشه) فایل‌های ترجمه‌شده را مستقیماً در پوشهٔ نصب کپی می‌کند:

۱. **پوشهٔ نصب** بازی را انتخاب کنید
۲. پیش از بازنویسی یک فایل موجود، در کنار فایل اصلی یک **پشتیبان `.bak`** ساخته می‌شود
۳. **ایمنی**: اگر پشتیبان‌گیری از یک فایل ناموفق باشد، آن فایل **رد می‌شود** (هرگز بدون نسخهٔ ایمن بازنویسی نمی‌شود)
۴. در پایان می‌بینید چند فایل نوشته و چند فایل رد شده است

برای بازگرداندن فایل اصلی، کافی است نام فایل `.bak` را با حذف پسوند تغییر دهید.

### انتشار (از پیش پرشده)

دکمهٔ **«انتشار در Patch Hub»** (آیکون اشتراک‌گذاری) ترجمه را برای انجمن می‌فرستد. این کار **فایل ترجمه‌شدهٔ واقعی** را منتشر می‌کند (نه فقط یک مانیفست فراداده‌ای)، با نام، بازی، زبان‌ها و درصد تکمیل که **از پیش پر شده‌اند**. بسته پیش از آنکه به‌صورت عمومی قابل مشاهده شود، وارد **صف بازبینی** می‌شود.

> انتشار نیازمند ورود به **Community Hub** است (حسابی جدا از پروفایل محلی شما).

### کاوش / پچ‌های من

صفحهٔ **Patch Hub** دو تب دارد:

- **کاوش** — همهٔ بسته‌های منتشرشده توسط انجمن، با جست‌وجو و مرتب‌سازی (بیشترین دانلود، بالاترین امتیاز، به‌روزرسانی اخیر، میزان تکمیل)
- **پچ‌های من** — فقط بسته‌هایی که با پروفایل فعلی منتشر کرده‌اید

### کارایی

فهرست‌های Patch Hub از یک **کش محلی** (حدود ۶۰ ثانیه) استفاده می‌کنند: جابه‌جایی رفت‌وبرگشتی میان تب‌ها دیگر هر بار داده‌ها را از سرور بارگذاری نمی‌کند. کش پس از انتشار یا دانلود به‌طور خودکار تازه می‌شود. انتشار و گزارش نیز یک **فاصلهٔ زمانی حداقلی** کوچک میان ارسال‌ها دارند تا سرور مشترک بیش از حد بارگذاری نشود.

---

## OLLAMA INFERENCE PARAMETERS
## (پارامترهای استنتاج Ollama)

*(جدید در v1.15.0)*

هنگامی که با یک **مدل محلی Ollama** ترجمه می‌کنید، می‌توانید کنترل کنید که مدل *چگونه* متن تولید کند. صفحه را از **Ollama Manager → «پیشرفته»** (مسیر `/ollama-manager/advanced`) باز کنید.

> اگر چیزی را تغییر ندهید، مقادیر پیش‌فرض توصیه‌شده فعال می‌مانند: ترجمه دقیقاً مانند قبل عمل می‌کند. پارامترهای پیشرفته **فقط** زمانی که آن‌ها را تنظیم کنید به Ollama ارسال می‌شوند.

### پیش‌تنظیم‌ها

سه پروفایل آمادهٔ استفاده، تنظیم‌شده برای ترجمهٔ بازی‌های ویدیویی:

| پیش‌تنظیم | Temperature | `num_ctx` | چه زمانی استفاده شود |
|-----------|-------------|-----------|----------------------|
| 🎯 **وفادار** | 0.1 | 8192 | حداکثر وفاداری به متن، کمترین ابتکار. برای ترجمه **توصیه می‌شود**. |
| ⚖️ **متوازن** | 0.3 | 4096 | مصالحه‌ای میان وفاداری و طبیعی بودن. |
| ✨ **خلاق** | 0.7 | 4096 | آزادی سبکی بیشتر: برای دیالوگ و لحن رسا. |

### حالت حرفه‌ای

**«حالت حرفه‌ای»** را باز کنید تا هر پارامتر را با اسلایدر تنظیم کنید:

| پارامتر | اثر |
|---------|-----|
| `temperature` | تصادفی بودن تولید. پایین‌تر = قطعی‌تر و وفادارتر؛ بالاتر = متنوع‌تر و خلاق‌تر. |
| `top_p` | *Nucleus sampling*: فقط محتمل‌ترین توکن‌ها تا این جرم احتمال در نظر گرفته شوند. پایین‌تر = متمرکزتر. |
| `top_k` | محدود کردن انتخاب به k توکن محتمل‌تر در هر گام. پایین‌تر = انحراف کمتر. |
| `repeat_penalty` | تکرار واژه‌ها را کاهش می‌دهد. بالای 1.0 حلقه‌ها را کم می‌کند؛ خیلی زیاد متن را مخدوش می‌کند. |
| `num_ctx` | پنجرهٔ زمینه (توکن‌های نگه‌داشته در حافظه). **گسترده‌تر = بدون کوتاه‌شدن در متن‌های بلند**، اما RAM/VRAM بیشتر. |
| `seed` *(اختیاری)* | برای نتایج **قابل بازتولید** بذر تصادفی را ثابت می‌کند (برای بنچمارک مفید است). خاموش = هر اجرا ممکن است متفاوت باشد. |

ویرایش هر مقدار، پیش‌تنظیم را به‌طور خودکار به **«custom»** تغییر می‌دهد.

### چه زمانی و چرا از آن‌ها استفاده کنیم

- **وفاداری در برابر خلاقیت**: برای ترجمه، **temperature پایین** (پیش‌تنظیم وفادار) را ترجیح دهید — ابتکار کمتر، وفاداری بیشتر. فقط برای دیالوگی که لحن رساتر می‌خواهید آن را بالا ببرید.
- **فایل‌های بلند**: اگر هنگام ترجمهٔ متن‌های بلند متوجه کوتاه‌شدن یا از دست رفتن زمینه شدید، **`num_ctx`** را افزایش دهید (به بهای RAM/VRAM بیشتر).
- **قابلیت بازتولید**: هنگامی که می‌خواهید دو مدل را مقایسه کنید یا همان ترجمه را دوباره اجرا کرده و دقیقاً همان خروجی را بگیرید، **`seed`** را فعال کنید.

### نحوهٔ استفاده

۱. **Ollama Manager** را از نوار کناری باز کرده و روی **«پیشرفته»** کلیک کنید
۲. یک پیش‌تنظیم انتخاب کنید (برای ترجمه از **وفادار** شروع کنید) یا **حالت حرفه‌ای** را باز کنید
۳. پارامترها را تنظیم کنید (مثلاً برای فایل‌های بلند `num_ctx` را بالا ببرید، برای نتایج تکرارپذیر `seed` را فعال کنید)
۴. روی **«ذخیره»** کلیک کنید — پارامترها برای ترجمه‌های بعدی ذخیره می‌شوند

> **دامنه**: این پارامترها بر ترجمه‌های اجراشده با **مدل‌های محلی Ollama** (از جمله گذر *reflection*) اعمال می‌شوند. آن‌ها بر ارائه‌دهندگان وب (Gemini، Groq و غیره) تأثیری ندارند.
...
می‌توانید مستقیماً از داخل برنامه و بدون ترک GameStringer یک باگ را گزارش دهید، ایده‌ای پیشنهاد کنید یا نظرتان را به ما بگویید.

### نحوهٔ ارسال

۱. به **تنظیمات → انجمن** (تب Community Hub) بروید
۲. در کادر **«ارسال بازخورد»** روی **«ارسال بازخورد»** کلیک کنید
۳. یک **دسته** انتخاب کنید — 🐞 باگ، 💡 ایده یا 💬 سایر
۴. پیام خود را بنویسید و روی **«ارسال»** کلیک کنید

### زمینه‌ای که به‌طور خودکار پیوست می‌شود

برای کمک به درک بهتر مشکل، یک **زمینهٔ فنی حداقلی** به پیام پیوست می‌شود:

- **نسخهٔ** برنامه
- **پلتفرم** (سیستم‌عامل + Tauri/Web)
- **صفحه‌ای** که در آن بودید (مسیر فعلی)
- **بازی** (فقط اگر روی یک عنوان مشخص کار می‌کردید)

> **حریم خصوصی**: هیچ داده شخصی‌ای نیست. فقط نسخه، پلتفرم و صفحهٔ فعلی.

### اگر بک‌اند در دسترس نباشد

این ویجت **fail-open** است: اگر سرور پیکربندی نشده باشد یا آفلاین باشید، پیام از دست نمی‌رود. همچنان می‌توانید با یک کلیک **گزارش را در کلیپ‌بورد کپی کنید** یا آن را با **ایمیل** بفرستید — زمینهٔ فنی از پیش در متن گنجانده شده است.
...
GameStringer v1.15.0 - راهنما به‌روزرسانی شده در ۲۴/۰۷/۲۰۲۶
