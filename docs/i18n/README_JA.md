...
### 🆕 v1.15.0 の新機能 — Ollama パラメータと プロジェクト ↔ Patch Hub
...
- **Auto-Select Engine**（NEW v1.9.0）：対象言語 + ゲームジャンルに基づいてプロバイダーを動的にランク付けするプリセット
...
- **Voice Pipeline**：speech-to-text → 翻訳 → text-to-speech、**Duration Matching**（NEW v1.9.0）付き — 元の音声の長さに合わせて速度を自動調整
- **Lip Sync**（NEW v1.9.0）：ビスム生成のためのRhubarb統合、Unity/Unreal向けエクスポート
- **Gridly CSV Export/Import**（NEW v1.9.0）：Gridly/Lokalise/Crowdin対応の多言語フォーマット
...
- **Video Extractor**（v1.9.0）：AIアップスケーリング付きでレトロ/モダンゲームからFMVビデオを抽出・変換
