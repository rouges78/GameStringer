...
2. **Ваша игровая библиотека загружается автоматически** — Steam, Epic, GOG, Origin, Ubisoft, Amazon, itch.io, Humble App, Game Jolt, Big Fish (установленные игры обнаруживаются за секунды, без ограничения по количеству)
...
После запуска P.T. на нескольких играх откройте **P.T.Rank**, чтобы увидеть все проанализированные тайтлы, отсортированные по сложности. Идеально для планирования очереди перевода: начните с лёгких побед, а RPG с сотнями тысяч строк оставьте напоследок.

### Dry Run Scanner

Не хотите анализировать игры по одной? Запустите **Dry Run** со страницы Библиотеки, чтобы сканировать **всю установленную библиотеку пакетно**, без ограничения по числу игр,, с **нулевыми изменениями файлов**. Вы получите JSON-отчёт, категоризирующий каждую игру как **Ready** (движок поддерживается + строки извлекаемы), **Errors** (проблемы манифеста / блокировщик DRM) или **Unsupported** (неизвестный движок / нет текста). Прогресс в реальном времени, а резервная копия не нужна, потому что ничто не трогается.
...
У GameStringer **12 движков с выделенным парсером**, а всего он распознаёт **более 20**, с различными уровнями глубины:
...
- **Dry Run Scanner** — пакетное сканирование всей установленной библиотеки, без ограничения по числу игр, без изменений
...
- **Вся установленная библиотека** распознаётся за секунды, без ограничения по числу игр
