...
impl NotificationManager {
    ...
    fn apply_custom_sorting(&self, notifications: &mut Vec<Notification>, filter: &NotificationFilter) {
 ...
 ===== SYSTEM NOTIFICATION METHODS =====
    ...
    pub async fn create_system_broadcast_notification(
        ...
        // Ottieni tutti i profili attivi
        let active_profiles = self.get_active_profiles().await?;
        let mut created_notifications = Vec::new();
        
        // Crea metadati per notifica di sistema
        let metadata = NotificationMetadata {
            source: "system".to_string(),
            category: "broadcast".to_string(),
            tags: vec!["system".to_string(), "broadcast".to_string()],
            custom_data: Some({
                let mut data = HashMap::new();
                data.insert("broadcast_id".to_string(), serde_json::Value::String(Uuid::new_v4().to_string()));
                data.insert("created_at".to_string(), serde_json::Value::String(Utc::now().to_rfc3339()));
                data
            }),
        };
        
        // Crea la notifica per ogni profilo attivo
        for profile_id in active_profiles {
            ...
            match self.create_notification(request).await {
                ...
                Err(e) => {
                    println!("[SYSTEM NOTIFICATION] Errore creazione notifica per profilo {}: {}", profile_id, e);
                }
            }
        }
        
        println!("[SYSTEM NOTIFICATION] Broadcast creato per {} profili", created_notifications.len());
        Ok(created_notifications)
    }

    /// Crea una notifica di sistema con priorità urgente
    ...
    pub async fn create_maintenance_notification(
        ...
        let duration_hours = (maintenance_end - maintenance_start).num_hours();
        let title = "Manutenzione Programmata".to_string();
        let message = format!(
            "Manutenzione programmata dalle {} alle {} (durata: {} ore). {}",
            maintenance_start.format("%d/%m/%Y %H:%M"),
            maintenance_end.format("%d/%m/%Y %H:%M"),
            duration_hours,
            description
        );
        
        // La notifica scade 1 ora dopo la fine della manutenzione
        let expires_at = Some(maintenance_end + chrono::Duration::hours(1));
    ...
    /// Crea notifica di aggiornamento disponibile
    pub async fn create_update_available_notification(
        ...
        let title = format!("Aggiornamento Disponibile - v{}", version);
        let message = format!("È disponibile la versione {}. {}", version, release_notes);
        
        // La notifica scade dopo 30 giorni
    ...
    pub async fn get_active_system_notifications(&self) -> NotificationResult<Vec<Notification>> {
        let filter = NotificationFilter {
            ...
            priority: None,
            unread_only: None,
            category: Some("broadcast".to_string()),
            limit: None,
            offset: None,
        };
        
        // Ottieni da tutti i profili (questo richiede una query speciale)
    ...
    pub async fn get_system_notification_read_status(
        &self,
        notification_id: &str,
    ) -> NotificationResult<crate::notifications::models::SystemNotificationReadStatus> {
    ...
    pub async fn expire_old_system_notifications(&self, days_old: u32) -> NotificationResult<u32> {
        ...
        self.storage.expire_old_system_notifications(cutoff_date).await
    }
    ...
    /// Ottiene lista profili attivi (helper method)
    async fn get_active_profiles(&self) -> NotificationResult<Vec<String>> {
        // Per ora restituiamo tutti i profili che hanno preferenze
        // In futuro potremmo integrare con il ProfileManager per ottenere solo quelli attivi
        self.storage.get_all_profile_ids().await
    }