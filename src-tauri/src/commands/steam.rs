...
//! 
//! Questo modulo gestisce tutte le interazioni con Steam:
//! - **Sezione 1**: Imports e Strutture Dati (righe 1-140)
//! - **Sezione 2**: Rate Limiter e API Utils (righe 140-260)
//! - **Sezione 3**: Debug API Commands (righe 260-660)
//! - **Sezione 4**: Game Detection (VR, Engine, Genres, Languages) (righe 660-1200)
//! - **Sezione 5**: Credentials & Encryption (righe 1200-1620)
//! - **Sezione 6**: Steam Config & Connection (righe 1620-1910)
//! - **Sezione 7**: Steam Games Library (righe 1910-2460)
//! - **Sezione 8**: Credentials Commands (righe 2460-2930)
//! - **Sezione 9**: Local Games & ACF Parsing (righe 2930-3960)
...
// ============================================================================
// SEZIONE 2: RATE LIMITER E API UTILS
// ============================================================================
    ...
    loop {
        ...
        if !RATE_LIMITER.is_request_allowed(endpoint) {
            if retry_count >= MAX_RETRIES {
                return Err(format!("Rate limit exceeded for endpoint: {} after {} retries", endpoint, retry_count));
            }
            warn!("[RateLimit] Rate limited, waiting {}ms before retry {}/{}", delay_ms, retry_count + 1, MAX_RETRIES);
        ...
        match client.get(url).send().await {
            Ok(response) => {
                ...
                // Handle rate limit responses (403, 429)
                if status.as_u16() == 403 || status.as_u16() == 429 {
                    if retry_count >= MAX_RETRIES {
                        return Err(format!("API returned {} after {} retries", status, retry_count));
                    }
                    
                    warn!("[RateLimit] Received {} from Steam API, exponential backoff: {}ms (retry {}/{})", 
                          status, delay_ms, retry_count + 1, MAX_RETRIES);
                    
                    tokio::time::sleep(Duration::from_millis(delay_ms)).await;
                    delay_ms = (delay_ms * 2).min(60000); // Max 60 seconds for 403/429
                    retry_count += 1;
                    continue;
                }
            ...
            Err(e) => {
                ...
                warn!("[RateLimit] Request error, retrying in {}ms: {}", delay_ms, e);
                tokio::time::sleep(Duration::from_millis(delay_ms)).await;
...
/// Quick rate-limited request without retries (for non-critical calls)
#[allow(dead_code)]
async fn make_quick_request(
    client: &reqwest::Client,
    url: &str,
    endpoint: &str,
) -> Result<reqwest::Response, String> {
...
pub async fn debug_steam_api(api_key: String, steam_id: String) -> Result<String, String> {
    ...
    let url = format!(
        "https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key={}&steamid={}&format=json&include_appinfo=true&include_played_free_games=true&skip_unvetted_apps=false",
        actual_key, actual_id
    );
...
// ============================================================================
// SEZIONE 4: GAME DETECTION (VR, Engine, Genres, Languages)
// ============================================================================
...
fn detect_game_engine(name: &str, _appid: u32) -> String {
    // Usa il rilevamento centralizzato
    crate::engine_detector::detect_engine_by_name(name).unwrap_or_else(|| "Unknown".to_string())
}
...
fn detect_supported_languages(name: &str, appid: u32) -> String {
    ...
    // 🎮 GIOCHI UNITY/INDIE CON BUONA LOCALIZZAZIONE (5-15 lingue)
    let unity_well_localized = [
        "what remains of edith finch", "firewatch", "subnautica", "the forest",
        "among us", "fall guys", "phasmophobia", "valheim", "satisfactory",
        "kerbal space program", "cities skylines", "hollow knight", "cuphead",
        "ori and", "celeste", "hades", "dead cells", "slay the spire", 
        "inscryption", "cult of the lamb", "outer wilds", "disco elysium",
        "pillars of eternity", "divinity original sin", "baldur's gate",
        "untitled goose game", "a short hike", "return of the obra dinn",
        "escape from tarkov", "rust", "devour", "lethal company", "content warning",
        "potion craft", "the beast inside", "little nightmares", "arizona sunshine",
        "job simulator", "vrchat", "beat saber"
    ];
    
    // 🇺🇸 GIOCHI INDIE CON LOCALIZZAZIONE LIMITATA (solo EN/FR/DE)
    let indie_limited = [
        "the binding of isaac", "risk of rain", "enter the gungeon",
        "spelunky", "super meat boy", "fez", "braid", "limbo", "inside", "playdead",
        "control", "alan wake", "max payne", "quantum break", "remedy",
        "dishonored", "prey", "deathloop", "arkane",
    ...
    // 🎮 CONTROLLO: Giochi Unity/Indie con BUONA localizzazione
    for game in &unity_well_localized {
        if name_lower.contains(game) {
            return "English,Italian,French,German,Spanish,Portuguese,Russian,Japanese,Korean,Chinese,Polish".to_string();
        }
    }
    
    // 🇺🇸 CONTROLLO: Indie con localizzazione MOLTO limitata
    for game in &indie_limited {
        if name_lower.contains(game) {
            return "English,French,German".to_string();
        }
    }
...
// ============================================================================
// SEZIONE 5: CREDENTIALS & ENCRYPTION
// ============================================================================
...
fn verify_credential_integrity(api_key: &str, steam_id: &str) -> Result<(), String> {
    ...
    if !api_key.chars().all(|c| c.is_ascii_alphanumeric()) {
        return Err("API key must contain only alphanumeric characters".to_string());
    }
...
// ============================================================================
// SEZIONE 6: STEAM CONFIG & CONNECTION
// ============================================================================
    ...
    match manager.load_credential_for_active_profile(crate::profiles::StoreType::Steam).await {
        Ok(Some(credential)) => {
            ...
            if let Err(e) = verify_credential_integrity(&credential.password, &steam_id) {
                return Ok(TestConnectionResult {
                    connected: false,
                    games_count: None,
                    error: Some(format!("Credenziali Steam non valide: {}", e)),
                });
            }
            
            // Chiama l'API Steam per ottenere il conteggio reale dei giochi owned
            let api_key = &credential.password;
            let url = format!(
                "https://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key={}&steamid={}&format=json&include_appinfo=false&include_played_free_games=true&skip_unvetted_apps=false",
                api_key, steam_id
            );
                ...
                Ok(response) => {
                    if response.status().is_success() {
                        if let Ok(json) = response.json::<serde_json::Value>().await {
                            let games_count = json["response"]["game_count"].as_u64().unwrap_or(0) as u32;
                            debug!("[RUST] ✅ Steam API test passed! {} giochi owned", games_count);
                            ...
                            // Fallback: usa giochi installati
                            let installed_games = get_installed_steam_app_ids().await;
                            Ok(TestConnectionResult {
                                connected: true,
                                games_count: Some(installed_games.len() as u32),
                                error: None,
                            })
                        }
                    } else {
                        Ok(TestConnectionResult {
                            connected: false,
                            games_count: None,
                            error: Some(format!("API Steam error: {}", response.status())),
                        })
                    }
                },
                Err(e) => {
                    // Timeout o errore di rete - usa fallback locale
                    let installed_games = get_installed_steam_app_ids().await;
                    debug!("[RUST] ⚠️ API timeout, usando {} giochi installati", installed_games.len());
                    Ok(TestConnectionResult {
                        connected: true,
                        games_count: Some(installed_games.len() as u32),
                        error: Some(format!("API timeout: {}", e)),
                    })
                }
            }
        },
        Ok(None) => {
            ...
            Ok(TestConnectionResult {
                connected: false,
                games_count: None,
                error: Some("Nessuna credenziale Steam salvata".to_string()),
            })
        },
        Err(e) => {
            Ok(TestConnectionResult {
                connected: false,
                games_count: None,
                error: Some(format!("Errore test connessione Steam: {}", e)),
            })
        }
    }
}
...
pub async fn get_steam_games(
    ...
    let file_content = match std::fs::read_to_string(file_path) {
        ...
        Err(e) => {
            // Se il file non esiste, ritorna array vuoto invece di errore
            debug!("[RUST] ⚠️ steam_owned_games.json non trovato, ritorno array vuoto: {}", e);
            return Ok(vec![]);
        }
    };
    ...
    let games_data: Value = match serde_json::from_str(&file_content) {
        ...
        Err(e) => {
            // Se il JSON è invalido, ritorna array vuoto
            debug!("[RUST] ⚠️ JSON parse error, ritorno array vuoto: {}", e);
            return Ok(vec![]);
        }
    };
...
// ============================================================================
// SEZIONE 8: CREDENTIALS COMMANDS
// ============================================================================
...
pub async fn load_steam_credentials(
    ...
    match manager.load_credential_for_active_profile(crate::profiles::StoreType::Steam).await {
        Ok(Some(credential)) => {
            ...
            // Restituisci direttamente senza ri-criptare (già gestito dal ProfileManager)
            let credentials = SteamCredentials {
                api_key_encrypted: credential.password.clone(), // API key già salvata
                steam_id: steam_id.clone(),
                saved_at: credential.created_at.to_rfc3339(),
                nonce: "profile_managed".to_string(), // Placeholder - gestito dal profilo
            };
        ...
        Ok(None) => {
            // Nessuna credenziale - restituisci struct vuota invece di errore
            debug!("[RUST] ⚠️ Nessuna credenziale Steam nel profilo attivo");
            Ok(SteamCredentials {
        ...
        Err(e) => {
            debug!("[RUST] ⚠️ Errore caricamento credenziali: {}", e);
            // Restituisci struct vuota invece di errore per non bloccare l'UI
            Ok(SteamCredentials {
                api_key_encrypted: String::new(),
                steam_id: String::new(),
                saved_at: String::new(),
...
pub async fn auto_connect_steam(
    ...
    // Se nonce è "profile_managed", l'API key è già in chiaro (gestita dal ProfileManager)
    // Altrimenti, prova a decrittare (sistema legacy)
    let api_key = if credentials.nonce == "profile_managed" {
        debug!("[RUST] Using profile-managed API key (already decrypted)");
        credentials.api_key_encrypted.clone()
    } else if credentials.nonce.is_empty() {
        // Nonce vuoto = credenziali vuote o corrotte
        return Err("Credenziali Steam non configurate".to_string());
    } else {
        // Sistema legacy con crittografia
        match decrypt_api_key(&credentials.api_key_encrypted, &credentials.nonce) {
            Ok(key) => key,
            Err(e) => {
    ...
    match get_steam_games(api_key, credentials.steam_id, Some(false), profile_state).await {
        Ok(games) => {
...
/// Ora rileva anche giochi condivisi tramite Family Sharing
fn parse_acf_file_custom(content: &str) -> Result<LocalGameInfo, String> {
    ...
    for line in content.lines() {
        ...
        } else if line.contains("\"LastOwner\"") {
            last_owner = extract_quoted_value(line);
        }
    }
    ...
    // StateFlags: 4 = fully installed, 1026 = shared content
    // LastOwner diverso da 0 indica che il gioco potrebbe essere condiviso
    let is_shared = state_flags.map(|f| f & 1024 != 0).unwrap_or(false) 
                    || last_owner.as_ref().map(|o| !o.is_empty() && o != "0").unwrap_or(false);
    
    let status = if is_shared {
        GameStatus::Shared { from_steam_id: last_owner.unwrap_or_default() }
    } else {
        GameStatus::Installed { path: install_path }
    };
