//! # Epic Games Integration Module
//! 
//! Questo modulo gestisce tutte le interazioni con Epic Games Store:
//! - **Sezione 1**: Imports e Strutture Dati (righe 1-80)
//! - **Sezione 2**: Game Details & Covers API (righe 80-180)
//! - **Sezione 3**: OAuth Flow & Authentication (righe 180-470)
//! - **Sezione 4**: Games Library (righe 470-640)
//! - **Sezione 5**: Legendary CLI Integration (righe 640-870)
//! - **Sezione 6**: Disconnect & Cache (righe 870-1000)
//! - **Sezione 7**: Web API & Account Search (righe 1000-1300)
//! - **Sezione 8**: Local Files & Registry (righe 1300-2561)
...
// ============================================================================
// SEZIONE 2: GAME DETAILS & COVERS API
// ============================================================================
...
// ============================================================================
// SEZIONE 3: OAUTH FLOW & AUTHENTICATION
// ============================================================================
    ...
    match std::process::Command::new("legendary").args(&["status"]).output() {
        Ok(output) => {
            ...
            if status_text.contains("Epic account:") && !status_text.contains("not logged in") {
                println!("[EPIC] ✅ Legendary autenticato!");
                result["legendary_authenticated"] = serde_json::Value::Bool(true);
                
                // Ottieni lista giochi da Legendary
                match try_legendary_library("").await {
                    Ok(games) => {
                        result["connected"] = serde_json::Value::Bool(true);
                        result["games_count"] = serde_json::Value::Number(games.len().into());
                        result["status"] = serde_json::Value::String(format!("Epic Games connesso via Legendary - {} giochi", games.len()));
                        return Ok(result);
                    }
                    Err(e) => {
                        println!("[EPIC] ⚠️ Legendary autenticato ma errore lista giochi: {}", e);
                        // Continua comunque, mostra come connesso
                        result["connected"] = serde_json::Value::Bool(true);
                        result["status"] = serde_json::Value::String("Epic Games connesso via Legendary".to_string());
                        return Ok(result);
                    }
                }
            }
        ...
        Err(e) => {
            println!("[EPIC] ℹ️ Legendary non disponibile: {}", e);
        }
    }
    
    // Fallback: prova credenziali salvate
...
pub async fn get_epic_games_complete() -> Result<Vec<GameInfo>, String> {
    ...
    // Metodo 1: Prova PRIMA con Legendary (non richiede credenziali salvate)
    match try_legendary_library_direct().await {
        Ok(legendary_games) => {
            println!("[EPIC] ✅ Legendary direct: {} giochi trovati", legendary_games.len());
            return Ok(legendary_games);
        }
        Err(e) => {
            println!("[EPIC] ⚠️ Legendary direct non disponibile: {}", e);
        }
    }
    
    // Metodo 2: Prova con credenziali salvate
    match get_epic_owned_games().await {
        Ok(owned_games) => {
...
/// Prova a usare Legendary direttamente senza credenziali salvate
async fn try_legendary_library_direct() -> Result<Vec<GameInfo>, String> {
    ...
    if !output.status.success() {
        return Err("Legendary non autenticato o errore".to_string());
    }
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    
    // Legendary può restituire sia un array che un oggetto
    let json: serde_json::Value = serde_json::from_str(&stdout)
        .map_err(|e| format!("Errore parsing JSON Legendary: {}", e))?;
    
    let mut games = Vec::new();
    
    // Caso 1: Array di giochi (formato più comune)
    if let Some(games_arr) = json.as_array() {
        println!("[EPIC] Parsing array con {} elementi", games_arr.len());
        for game_data in games_arr {
            // Cerca il titolo in vari campi possibili
            let title = game_data["app_title"].as_str()
                .or_else(|| game_data["title"].as_str())
                .or_else(|| game_data["app_name"].as_str());
            
            let app_name = game_data["app_name"].as_str()
                .or_else(|| game_data["id"].as_str());
            ...
            if let (Some(title), Some(app_name)) = (title, app_name) {
                ...
                // Cerca immagine header
                let header_image = game_data["metadata"]["keyImages"]
                    .as_array()
                    .and_then(|imgs| imgs.iter().find(|img| img["type"].as_str() == Some("DieselGameBoxTall")))
    ...
    // Caso 2: Oggetto con chiavi = app_name (formato legacy)
        ...
        for (app_name, game_data) in games_obj {
            if let Some(title) = game_data["title"].as_str()
                ...
                let is_installed = game_data["install_path"].as_str().is_some() 
                    || game_data["installed"].as_bool().unwrap_or(false);
                ...
                games.push(GameInfo {
                    ...
                    icon: None,
                    image_url: None,
                    header_image: game_data["metadata"]["keyImages"]
                        .as_array()
                        .and_then(|imgs| imgs.iter().find(|img| img["type"].as_str() == Some("DieselGameBoxTall")))
                        .and_then(|img| img["url"].as_str())
                        .map(|s| s.to_string()),
    ...
    if games.is_empty() {
        return Err("Nessun gioco trovato con Legendary".to_string());
    }
    
    println!("[EPIC] ✅ Legendary direct: {} giochi", games.len());
    Ok(games)
}
...
// ============================================================================
// SEZIONE 5: LEGENDARY CLI INTEGRATION
// ============================================================================
            ...
            if output.status.success() {
                ...
                match std::process::Command::new("legendary").args(&["status"]).output() {
                    Ok(status_output) => {
                        ...
                        // Legendary mostra "Epic account: username" quando autenticato
                        if (status_text.contains("Epic account:") || status_text.contains("Epic Games account")) 
...
pub async fn authenticate_legendary() -> Result<String, String> {
    ...
    match std::process::Command::new("legendary").args(&["auth", "--import"]).output() {
        Ok(output) => {
            if output.status.success() {
                let stdout = String::from_utf8_lossy(&output.stdout);
                if stdout.contains("Successfully") || stdout.contains("logged in") {
                    println!("[EPIC] ✅ Sessione importata con successo!");
                    return Ok("✅ Sessione Epic Games importata automaticamente! I tuoi giochi sono ora disponibili.".to_string());
                }
            }
            println!("[EPIC] ⚠️ Import fallito, provo con browser...");
        }
        Err(e) => {
            println!("[EPIC] ⚠️ Errore import: {}", e);
        }
    }
    
    // Fallback: apri il browser per autenticazione manuale
    // Usa --browser per aprire automaticamente il browser
    println!("[EPIC] 🌐 Apertura browser per autenticazione...");
    
    // Apri direttamente la pagina di login Epic
    let login_url = "https://legendary.gl/epiclogin";
    if let Err(e) = open::that(login_url) {
        println!("[EPIC] ⚠️ Errore apertura browser: {}", e);
    }
                            ...
                            match std::process::Command::new("legendary").args(&["status"]).output() {
                                Ok(status_output) => {
                                    ...
                                    if status_text.contains("Epic Games account") && !status_text.contains("not logged in") {
                                        return Ok("✅ Autenticazione completata con successo! Ora puoi accedere alla tua libreria Epic Games completa.".to_string());
                                    } else {
                                        return Ok("⚠️ Processo completato, ma autenticazione non confermata. Riprova.".to_string());
                                    }
                                }
                                Err(_) => {
                                    return Ok("✅ Processo di autenticazione completato. Riavvia GameStringer per verificare.".to_string());
                                }
                            }
                        } else {
                            return Err("❌ Autenticazione fallita. Riprova.".to_string());
                        }
                    }
                    Ok(None) => {
                        ...
                        if start.elapsed() > timeout {
                            let _ = child.kill();
                            return Err("⏱️ Timeout autenticazione. Il browser è stato aperto - completa l'accesso e copia il codice nel terminale.".to_string());
                        }
                        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
                    }
                    Err(e) => {
                        return Err(format!("❌ Errore durante l'autenticazione: {}", e));
                    }
                }
            }
        }
...
// ============================================================================
// SEZIONE 7: WEB API & ACCOUNT SEARCH
// ============================================================================
...
fn is_valid_epic_game(name: &str) -> bool {
    ...
    // 🚫 STRICT MODE: Se non è nella whitelist, RIFIUTA
    // Prima era troppo permissivo e accettava qualsiasi stringa "che sembrava un titolo"
    // Ora richiediamo che il gioco sia:
    // 1. Letto da un manifest .item valido (gestito altrove)
    // 2. O nella whitelist sopra
    // 3. O abbia caratteristiche MOLTO specifiche di un titolo di gioco
    
    // Pattern aggiuntivi per giochi indie/meno noti
    let indie_patterns = [
        "edition", "remastered", "deluxe", "goty", "definitive",
        "collection", "bundle", "pack", "dlc", "expansion",
        "episode", "chapter", "season", "part", "volume"
    ];
    
    for pattern in &indie_patterns {
        if name_lower.contains(pattern) {
            // Deve comunque avere caratteristiche di titolo
            let has_multiple_words = name.split_whitespace().count() >= 2;
    ...
    // Per tutto il resto: RIFIUTA
    // I giochi veri vengono dai manifest .item, non da parsing di file random
    false
}
