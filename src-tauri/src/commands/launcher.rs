...
pub async fn launch_steam_game(app_id: String) -> Result<LaunchResult, String> {
    ...
    // Metodo 1: Protocollo Steam (raccomandato)
    ...
    // Metodo 2: Fallback - Esecuzione diretta Steam
    match launch_steam_direct(&app_id).await {
        Ok(result) => {
            info!("✅ Gioco Steam {} avviato con successo via esecuzione diretta", app_id);
            Ok(result)
        }
        Err(e) => {
...
async fn launch_with_steam_protocol(steam_url: &str, app_id: &str) -> Result<LaunchResult, String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        {
            Ok(_) => {
                info!("✅ Protocollo Steam eseguito con successo");
                Ok(LaunchResult {
                    success: true,
            ...
            Err(e) => {
                Err(format!("Errore esecuzione protocollo Steam: {}", e))
            }
        }
    }
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        {
            Ok(_) => {
                info!("✅ Protocollo Steam eseguito con successo (Linux)");
                Ok(LaunchResult {
                    success: true,
                    message: format!("Gioco avviato tramite protocollo Steam: {}", steam_url),
                    method: "steam_protocol".to_string(),
