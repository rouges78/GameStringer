...
impl ProfileManager {
    ...
    pub async fn authenticate_profile(&mut self, name: &str, password: &str) -> ProfileResult<UserProfile> {
        ...
        match load_result {
            Ok(mut profile) => {
                ...
                // Sincronizza avatar dall'indice
                profile.avatar_path = profile_info.avatar_path.clone();
    ...
    pub async fn change_profile_password(&mut self, profile_id: &str, old_password: &str, new_password: &str) -> ProfileResult<()> {
        ...
        if new_password.len() < 4 {
             return Err(ProfileError::WeakPassword("La password deve avere almeno 4 caratteri".to_string()));
        }
        ...
        if let Some(profile) = &self.current_profile {
            ...
            match self.get_profile_info(&profile.id).await? {
                Some(info) => {
                    ...
                    Ok(info.name == profile.name && info.created_at == profile.created_at)
                }
                None => Ok(false)
            }
        } else {
    ...
    /// Aggiorna l'avatar del profilo
    pub async fn update_profile_avatar(&mut self, profile_id: &str, avatar_data: Option<String>) -> ProfileResult<()> {
        // Se c'è un dato avatar
        let avatar_path = if let Some(mut data) = avatar_data {
            // Rimuovi prefisso 'custom:' se presente (usato dal frontend)
            if data.starts_with("custom:") {
                data = data[7..].to_string();
            }

            // Se è un gradiente, usalo così com'è
            if data.starts_with("gradient-") {
                Some(data)
            } else if data.starts_with("data:image") {
                // Se è un data URL, estrai il base64
                let parts: Vec<&str> = data.split(',').collect();
                if parts.len() != 2 {
                    return Err(ProfileError::DataFormatError("Formato data URL non valido".to_string()));
                }
                
                // Decodifica base64
                let image_data = general_purpose::STANDARD.decode(parts[1])
                    .map_err(|e| ProfileError::DataFormatError(format!("Base64 non valido: {}", e)))?;
                ...
                // Assume filename (or legacy short string)
                Some(data)
            }
        } else {
            None
        ...
        println!("[PROFILE MANAGER] ✅ Avatar aggiornato per profilo {}", profile_id);
        Ok(())
    }
    ...
    pub async fn get_profile_avatar(&self, profile_id: &str) -> ProfileResult<Option<String>> {
        ...
        if let Some(avatar_path) = profile_info.avatar_path {
            // Se è un gradiente predefinito, restituisci quello
            if avatar_path.starts_with("gradient-") {
                return Ok(Some(avatar_path));
            }

            // Se è già un data URL, restituiscilo direttamente (legacy)
            if avatar_path.starts_with("data:image") {
                return Ok(Some(avatar_path));
            }

            // Se è raw base64 (lungo), restituiscilo come data URL (legacy)
            if avatar_path.len() > 200 && !avatar_path.contains('/') && !avatar_path.contains('\\') {
                return Ok(Some(format!("data:image/png;base64,{}", avatar_path)));
            }

            // Carica i dati dell'avatar dallo storage (filename)
            match self.storage.load_avatar(&avatar_path).await {
                Ok(data) => {
                    // Codifica in base64
                    let base64_data = general_purpose::STANDARD.encode(&data);
                    // Mime type semplice, default png
                    let mime = if avatar_path.ends_with(".jpg") || avatar_path.ends_with(".jpeg") { "image/jpeg" } else { "image/png" };
                    return Ok(Some(format!("data:{};base64,{}", mime, base64_data)));
                },
                Err(_) => {
                    // File non trovato, restituisci None
                    return Ok(None);
                }
            }
        }
