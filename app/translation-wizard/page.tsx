...
/**
 * Codifica un buffer binario in base64, che è il formato accettato dal comando
 * Tauri `save_binary_file`.
 *
 * A blocchi: `String.fromCharCode(...buf)` su un eseguibile da decine di MB
 * supera il limite di argomenti dello stack e va in RangeError.
 */
  ...
  const inlineBinaryPatch = async (gameCtx: GameContext, log: (m: string) => void) => {
    ...
    if (translatedCount > 0) {
      ...
      if (patchResult.success && patchResult.patchedBuffer) {
        ...
        try {
          // `write_binary_file` non è mai esistito lato Rust: il salvataggio
          // dell'eseguibile patchato falliva sempre, e l'errore finiva solo nel
          // log qui sotto. Il comando vero è save_binary_file, che accetta il
          // contenuto in base64.
          await invoke('save_binary_file', {
            filePath: outputPath,
            base64Content: toBase64(patchResult.patchedBuffer),
          });
  ...
  const inlineRenPy = async (gameCtx: GameContext, log: (m: string) => void) => {
    ...
    for (let fi = 0; fi < rpyFiles.length; fi++) {
      ...
      if (translated.size > 0) {
        ...
        // Era `create_directory`, comando inesistente: la cartella tl/<lingua>
        // non veniva mai creata e la scrittura del file di traduzione falliva
        // subito dopo. Il comando vero è ensure_directory (crea anche i parent).
        try { await invoke('ensure_directory', { path: tlDir }); } catch {}
