  ...
  const renderForm = (onSave: () => void, onCancel: () => void) => (
    <div className="rounded-sm bg-[#1b2838] border border-[#2a475e] p-4 space-y-3">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div>
          <label className="text-2xs text-[#8f98a0] uppercase tracking-wider font-semibold mb-1 block">{t('common.tag')}</label>
          <select 
            value={form.tag}
        ...
        <div>
          <label className="text-2xs text-[#8f98a0] uppercase tracking-wider font-semibold mb-1 flex items-center gap-1 ">
            <Gamepad2 className="h-3 w-3" /> {t('common.game')}
          </label>
          <Input 
            value={form.gameName}
            onChange={(e) => setForm(f => ({ ...f, gameName: e.target.value }))}
            placeholder={t('common.gameNameOptional')}
            className="h-8 text-xs bg-[#171d25] border-[#2a475e] text-[#c6d4df] placeholder:text-[#8f98a0]/40 focus:border-[#67c1f5]"
          />
        </div>
        <div>
          <label className="text-2xs text-[#8f98a0] uppercase tracking-wider font-semibold mb-1 flex items-center gap-1">
            <Image className="h-3 w-3" /> {t('common.imageUrl')}
          </label>
          <Input 
            value={form.image}
      ...
      <div>
        ...
        <Input 
          ...
          placeholder={t('common.newsTitle')}
      ...
      <div>
        ...
        <textarea 
          ...
          placeholder={t('common.newsDescription')}
          rows={3}
      ...
      {form.image && (
        <div className="flex items-center gap-3 p-2 rounded bg-[#171d25] border border-[#2a475e]/50">
          ...
          <span className="text-2xs text-[#8f98a0]">{t('common.imagePreview')}</span>
        </div>
      )}
    ...
    <div className="h-full overflow-y-auto custom-scrollbar p-4 space-y-3" style={{ background: 'linear-gradient(180deg, #1b2838 0%, #171d25 40%, #0e1419 100%)' }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-[#c6d4df]">News & {t('common.updates')}</h1>
            <p className="text-2xs text-[#8f98a0]">{t('common.manageTheDashboardNewsFeed')}</p>
          </div>
        </div>
      ...
      {posts.length === 0 && !isAdding && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm font-medium text-[#c6d4df]/60 mb-1">{t('common.noNewsYet')}</p>
          <p className="text-[11px] text-[#8f98a0]/40 mb-4">{t('common.createTheFirstNewsToPopulateTheDashboardFeed')}</p>
          <button 
